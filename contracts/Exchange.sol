...
contract Exchange is owned {
    ...
    function depositEtherB(string symbolName, uint amountTokens) public {
        ...
        ERC20Interface token = ERC20Interface(ethB);
        ...
        // // Overflow check
         require(balanceEthForAddress[msg.sender] + amountTokens >= balanceEthForAddress[msg.sender]);
        // // Credit the DEX token balance for the callinging address with the transferred amount
         balanceEthForAddress[msg.sender] += amountTokens;
    }
    ...
    function addEthBTokenAddress(address erc20TokenAddress) public onlyowner {
       ethB = erc20TokenAddress;
    }
    ...
    function hasTokenAddress(string symbolName) public constant returns (address) {
        uint8 index = getSymbolIndex(symbolName);

        return tokens[index].tokenContract;
    }
    ...
    function getSymbolIndexOrThrow(string symbolName) public constant returns (uint8) {
        uint8 index = getSymbolIndex(symbolName);
        require(index > 0);
        return index;
    }
    ...
    function depositToken(string symbolName, uint amountTokens) public {
        uint8 symbolNameIndexT = getSymbolIndexOrThrow(symbolName);
        // Check the Token Contract Address is initialised and not an uninitialised address(0) aka "0x0"
        require(tokens[symbolNameIndexT].tokenContract != address(0));

        ERC20Interface token = ERC20Interface(tokens[symbolNameIndexT].tokenContract);

        // Transfer an amount to this DEX from the calling address
         require(token.transferFrom(msg.sender, address(this), amountTokens) == true);
        // // Overflow check
         require(tokenBalanceForAddress[msg.sender][symbolNameIndexT] + amountTokens >= tokenBalanceForAddress[msg.sender][symbolNameIndex]);
        // // Credit the DEX token balance for the callinging address with the transferred amount
         tokenBalanceForAddress[msg.sender][symbolNameIndexT] += amountTokens;
        emit DepositForTokenReceived(msg.sender, symbolNameIndexT, amountTokens, now);
    }

    function withdrawToken(string symbolName, uint amountTokens) public {
        uint8 symbolNameIndexTemp = getSymbolIndexOrThrow(symbolName);
        require(tokens[symbolNameIndexTemp].tokenContract != address(0));

        ERC20Interface token = ERC20Interface(tokens[symbolNameIndexTemp].tokenContract);

        // Check sufficient balance to withdraw requested amount
        require(tokenBalanceForAddress[msg.sender][symbolNameIndexTemp] - amountTokens >= 0);
        // Overflow check to ensure future balance less than or equal to the current balance after deducting the withdrawn amount
        require(tokenBalanceForAddress[msg.sender][symbolNameIndexTemp] - amountTokens <= tokenBalanceForAddress[msg.sender][symbolNameIndex]);
        // Deduct amount requested to be withdrawing from the DEX Token Balance
        tokenBalanceForAddress[msg.sender][symbolNameIndexTemp] -= amountTokens;
        // Check that the `transfer` function of the Token Contract returns true
        require(token.transfer(msg.sender, amountTokens) == true);
        emit WithdrawalToken(msg.sender, symbolNameIndexTemp, amountTokens, now);
    }

    function getBalance(string symbolName) public constant returns (uint) {
        uint8 symbolNameIndexTemp = getSymbolIndexOrThrow(symbolName);
        return tokenBalanceForAddress[msg.sender][symbolNameIndexTemp];
    }
    ...
    function createBuyLimitOrderForTokensUnableToMatchWithSellOrderForBuyer(
        ...
        // Emit Event.
        emit LimitBuyOrderCreated(tokenNameIndex, msg.sender, amountOfTokensNecessary, priceInWei, tokens[tokenNameIndex].buyBook[priceInWei].offers_length);
    }
    ...
    function createSellLimitOrderForTokensUnableToMatchWithBuyOrderForSeller(
        ...
        // Emit Event
        emit LimitSellOrderCreated(tokenNameIndex, msg.sender, amountOfTokensNecessary, priceInWei, tokens[tokenNameIndex].sellBook[priceInWei].offers_length);
    }
    ...
    function cancelOrder(string symbolName, bool isSellOrder, uint priceInWei, uint offerKey) public {
        ...
        if (isSellOrder) {
            ...
            require(tokens[symbolNameIndexTemp].sellBook[priceInWei].offers[offerKey].who == msg.sender);
            // Obtain Tokens Amount that were to be sold in the Sell Limit Order
            uint tokensAmount = tokens[symbolNameIndexTemp].sellBook[priceInWei].offers[offerKey].amountTokens;
            // Overflow Check
            ...
            tokenBalanceForAddress[msg.sender][symbolNameIndexTemp] += tokensAmount;
        ...
        // Case 2: Cancel Buy Limit Order
            ...
            uint etherToRefund = tokens[symbolNameIndexTemp].buyBook[priceInWei].offers[offerKey].amountTokens * priceInWei;
            // Overflow Check
            ...
            tokens[symbolNameIndexTemp].buyBook[priceInWei].offers[offerKey].amountTokens = 0;
            emit BuyOrderCanceled(symbolNameIndexTemp, priceInWei, offerKey);
        }
    }
}
